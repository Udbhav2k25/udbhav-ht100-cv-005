...
# Maintain four traffic signals (e.g., for 4-way intersection)
traffic_signals = [TrafficSignal() for _ in range(4)]
...
def update_traffic_signals(has_emergency):
    """Update list of traffic signals based on emergency vehicle detection.

    If an emergency is detected, set all signals to GREEN (emergency priority).
    Otherwise, each signal cycles independently with a small offset so they
    don't all change at the same instant.
    """
    ...
    if has_emergency:
        for sig in traffic_signals:
            ...
            sig.state = TrafficSignal.GREEN
            sig.last_change = current_time
    else:
        for i, sig in enumerate(traffic_signals):
            # If this signal was in emergency mode, allow emergency_duration to elapse
            if sig.emergency_active:
                if current_time - sig.last_change > sig.emergency_duration:
                    sig.emergency_active = False
                    sig.state = TrafficSignal.YELLOW
                    sig.last_change = current_time
                continue

            # Normal independent cycle with small offset per signal
            # Offset ensures signals don't all switch at once
            offset = i * (sig.normal_duration / len(traffic_signals))
            elapsed = current_time - sig.last_change + offset
            if elapsed > sig.normal_duration:
                if sig.state == TrafficSignal.GREEN:
                    sig.state = TrafficSignal.YELLOW
                elif sig.state == TrafficSignal.YELLOW:
                    sig.state = TrafficSignal.RED
                else:  # RED
                    sig.state = TrafficSignal.GREEN
                sig.last_change = current_time
...
def process_traffic_frame(frame):
    ...
    if model is None:
        ...
        if not load_yolo_model():
            ...
            # Return frame and an empty state list if model unavailable
            return frame, [], False, [sig.state for sig in traffic_signals]
    ...
    try:
        ...
        # Update traffic signals (supports multiple signals)
        update_traffic_signals(emergency_detected)
        ...
        # Draw four traffic signals (top-right area)
        h, w = annotated_frame.shape[:2]
        base_x = w - 60
        base_y = 30
        spacing_x = 70
        signal_states = [sig.state for sig in traffic_signals]
        for i, sig in enumerate(traffic_signals):
            sx = base_x - i * spacing_x
            sy = base_y
            draw_traffic_signal(annotated_frame, sx, sy, sig.state, sig.emergency_active)
        ...
        return annotated_frame, vehicles, emergency_detected, signal_states
...
def process_traffic():
    ...
    try:
        ...
        annotated_frame, vehicles, emergency_detected, signal_states = process_traffic_frame(frame)
        ...
        return jsonify({
            ...
            'signal_state': signal_states[0] if isinstance(signal_states, (list, tuple)) and len(signal_states) > 0 else None,
...
def get_traffic_stats():
    ...
    # Aggregate signal states and emergency activity across all signals
    signal_states = [sig.state for sig in traffic_signals]
    emergency_any = any(sig.emergency_active for sig in traffic_signals)
    return jsonify({
        'stats': stats_copy,
        'signal_states': signal_states,
        'signal_state': signal_states[0] if signal_states else None,
        'emergency_active': emergency_any
    })


@app.route('/reset_stats', methods=['POST'])
def reset_stats():
    """Reset traffic statistics (called from client on page load)."""
    try:
        traffic_stats['total_vehicles'] = 0
        traffic_stats['emergency_vehicles'] = 0
        traffic_stats['detection_history'].clear()
        traffic_stats['emergency_logs'].clear()
...
def signal_control():
    """Manual traffic signal control. Accepts optional 'index' (0-3).

    If 'index' provided, set that signal only. Otherwise set all signals.
    """
    data = request.json
    state = data.get('state')
    index = data.get('index', None)

    if state not in [TrafficSignal.RED, TrafficSignal.YELLOW, TrafficSignal.GREEN]:
    ...
    if index is None:
        ...
        for sig in traffic_signals:
            sig.state = state
            sig.last_change = time.time()
        return jsonify({'success': True, 'state': state, 'index': None})

    try:
        idx = int(index)
        if idx < 0 or idx >= len(traffic_signals):
            return jsonify({'error': 'Invalid index'}), 400
        sig = traffic_signals[idx]
        sig.state = state
        sig.last_change = time.time()
        return jsonify({'success': True, 'state': state, 'index': idx})
    except Exception:
        return jsonify({'error': 'Invalid index'}), 400
...
def upload_image():
    ...
    if file and allowed_file(file.filename):
        ...
        try:
            ...
            # Process traffic frame
            annotated_frame, vehicles, emergency_detected, signal_states = process_traffic_frame(img)
